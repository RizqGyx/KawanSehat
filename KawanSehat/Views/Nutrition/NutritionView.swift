        ...
        NavigationStack {
            VStack(spacing: 0) {
                ...
                MacroSummaryWidget(
                    ...
                    foodBudget: userProfileVM.profile.dailyBudgetIDR
                )
                .padding()
...
// MARK: - Macro Summary Widget
struct MacroSummaryWidget: View {
    let proteinTarget: Double
    let carbsTarget: Double
    let fatTarget: Double
    let foodBudget: Double
    
    // Placeholder values - in production these would come from a MealLog service
    let proteinConsumed: Double = 0
    let carbsConsumed: Double = 0
    let fatConsumed: Double = 0
    let foodSpent: Double = 0
    
    var body: some View {
        VStack(spacing: 12) {
            // Title
            HStack {
                Label("Nutrisi Hari Ini", systemImage: "chart.pie.fill")
                    .font(.subheadline.bold())
                    .foregroundColor(.orange)
                Spacer()
            }
            
            // Macro Mini Cards in Grid
            HStack(spacing: 12) {
                // Protein
                VStack(spacing: 6) {
                    HStack(spacing: 2) {
                    ...
                    Text("Protein")
                        .font(.caption2.bold())
                        .foregroundStyle(.secondary)
                    ProgressView(value: min(proteinConsumed / proteinTarget, 1.0))
                        .tint(.red)
                }
                .frame(maxWidth: .infinity)
                .padding(.vertical, 8)
                ...
                VStack(spacing: 6) {
                    HStack(spacing: 2) {
                        Text(String(format: "%.0f", carbsConsumed))
                            .font(.headline.bold())
                            .foregroundColor(.blue)
                        Text("g")
                            .font(.caption2)
                    ...
                    Text("Karbo")
                        .font(.caption2.bold())
                        .foregroundStyle(.secondary)
                    ProgressView(value: min(carbsConsumed / carbsTarget, 1.0))
                        .tint(.blue)
                }
                .frame(maxWidth: .infinity)
                .padding(.vertical, 8)
                ...
                VStack(spacing: 6) {
                    HStack(spacing: 2) {
                        Text(String(format: "%.0f", fatConsumed))
                            .font(.headline.bold())
                    ...
                    Text("Lemak")
                        .font(.caption2.bold())
                        .foregroundStyle(.secondary)
                    ProgressView(value: min(fatConsumed / fatTarget, 1.0))
                        .tint(.yellow)
                }
                .frame(maxWidth: .infinity)
                .padding(.vertical, 8)
            ...
            // Budget info
            HStack {
                Label("Budget Makanan", systemImage: "creditcard")
                    .font(.caption.bold())
                    .foregroundColor(.green)
                Spacer()
                Text("Rp\(Int(foodSpent))/Rp\(Int(foodBudget))")
                    .font(.caption.bold())
            }
            ProgressView(value: min(foodSpent / foodBudget, 1.0))
                .tint(.green)
        }
    }
}
